...
public class PrincipleComponentAnalysis {
    ...
    public DenseMatrix64F V_t;
    public DenseMatrix64F U_t;
     ...
     *
    public void computeBasis( int numComponents, boolean calcu ) {
        ...
        SingularValueDecomposition<DenseMatrix64F> svd =
                DecompositionFactory.svd(A.numRows, A.numCols, calcu, true, false);
        if( !svd.decompose(A) )
            throw new RuntimeException("SVD failed");

        V_t = svd.getV(null,true);
        if( calcu ) U_t = svd.getU(null, true);
    ...
    public double[] getBasisVector( DenseMatrix64F sm, int which ) {
        if( which < 0 || which >= numComponents )
            throw new IllegalArgumentException("Invalid component");

        System.err.println( V_t.numCols + "  " + V_t.numRows );
        System.err.println( U_t.numCols + "  " + U_t.numRows );
        
        DenseMatrix64F v = new DenseMatrix64F(1,A.numRows);//,1);
        CommonOps.extract(sm,which,which+1,0,A.numRows,v,0,0);
