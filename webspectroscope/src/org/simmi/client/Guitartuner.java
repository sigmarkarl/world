...
public class Guitartuner implements EntryPoint {
	...
	public void tmp( Context2d context2d, double w52, double avg, Uint8ArrayNative ua, List<Integer> li, double w, double h, double prevbil, AudioContext acontext ) {
		/*int		maxi = 0;
						...
						if( ra != null ) {
							if( t++ % 1 == 0 ) {
								...
								for( int k = 0; k < fa.length(); k++ ) {
									float f = fa.get(k);
									context2d.lineTo( k*w/fa.length(), 140-f );
									context2d.lineTo( k*w/fa.length(), 140-f/*+val*/ );
									context2d.lineTo( k*w/fa.length(), 140-f );
								}
								context2d.closePath();
								context2d.stroke();
